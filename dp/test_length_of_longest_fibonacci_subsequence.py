import unittest
from length_of_longest_fibonacci_subsequence import (
    Solution,
    SolutionDP,
    SolutionOptimizedDP,
)
...
class TestLengthOfLongestFibonacciSubsequence(unittest.TestCase):
    def setUp(self):
        self.solution = (
            SolutionDP()
        )  # Using the DP solution as the reference implementation
    ...
    def test_large_numbers(self):
        ...
        expected = 5  # [2,8,10,18,28] forms a valid Fibonacci sequence
        self.assertEqual(self.solution.lenLongestFibSubseq(arr), expected)
        self.assertEqual(self.solution_optimized.lenLongestFibSubseq(arr), expected)

...
class Solution:
    ...
    # is a land cell.
    def _is_cell_land(self, x, y: int, grid: List[List[int]]):
        return grid[x][y] == 1

    # Traverse all cells of island starting at position (x, y) in 'grid2',
    # and check this island is a sub-island in 'grid1'.
    def _is_sub_island(self, x, y: int, grid1, grid2, visited: List[List[int]]) -> bool:
        ...
        while pending_cells:
            ...
            if not self._is_cell_land(curr_x, curr_y, grid1):
                is_sub_island = False
            ...
            for direction in self.DIRECTIONS:
                ...
                if (
                    ...
                    and self._is_cell_land(next_x, next_y, grid2)
                ):
    ...
    def countSubIslands(self, grid1: List[List[int]], grid2: List[List[int]]) -> int:
        ...
        for x in range(total_rows):
            for y in range(total_cols):
                ...
                if (
                    not visited[x][y]
                    and self._is_cell_land(x, y, grid2)
                    and self._is_sub_island(x, y, grid1, grid2, visited)
                ):
                    sub_island_counts += 1

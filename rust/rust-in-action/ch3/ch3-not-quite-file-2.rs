...
fn read(f: &mut File, save_to: &mut Vec<u8>) -> usize { // Returns the number of bytes read
    let mut tmp = f.data.clone(); // Makes a copy of the data here because save_to.append() shrinks the input Vec<T>

...
print_message $BLUE "🚀 Developer Aliases Installer"
print_message $BLUE "=============================="
...
if [[ "$SHELL" == *"zsh"* ]]; then
    ...
    print_message $YELLOW "⚠️  Unknown shell, defaulting to ~/.bashrc"
    SHELL_RC="$HOME/.bashrc"
    SHELL_NAME="bash"
fi

print_message $GREEN "✅ Detected shell: $SHELL_NAME"
print_message $BLUE "📝 Will install to: $SHELL_RC"

# Check if aliases file exists
ALIASES_FILE="$HOME/.dev_aliases"

if [[ -f "$ALIASES_FILE" ]]; then
    print_message $YELLOW "⚠️  Aliases file already exists at $ALIASES_FILE"
    read -p "Do you want to overwrite it? (y/N): " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        print_message $BLUE "ℹ️  Installation cancelled"
        exit 0
    fi
fi

# Create aliases file
print_message $BLUE "📝 Creating aliases file..."
...
print_message $BLUE "📋 Available alias categories:"
print_message $NC "  • Git: g, ga, gc, gp, etc."
print_message $NC "  • Kubernetes: k, kgp, kgs, etc."
print_message $NC "  • Docker: d, dp, dc, etc."
print_message $NC "  • Go: gor, got, gob, etc."

print_message $YELLOW "💡 Tips:"
print_message $NC "  • Run 'show-aliases' to see all aliases"
print_message $NC "  • Restart your terminal or run: source $SHELL_RC"
print_message $NC "  • Edit $ALIASES_FILE to customize"

print_message $BLUE "🚀 Quick test commands:"
